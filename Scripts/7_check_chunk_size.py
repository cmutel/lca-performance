...
for num_nodes in (10, 25, 50, 100, 200):
    profiler = Profiler()
    profiler.start()

    characterized_inventory = lca.characterization_matrix @ lca.biosphere_matrix

    for i in range(0, len(fus), num_nodes):
        i = 0
        chunk = fus[i:i + num_nodes]
    ...
    with open(Path(__file__).parent.parent / "Results" / f"chunked-{num_nodes}.html", "w") as f:
